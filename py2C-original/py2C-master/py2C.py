...
class I2c_device(object):
    ...
    def __init__(self,read_config=False,**kwargs):
        ...
        for kw in kwargs:
            if kw in self._default:
                setattr(self,kw,kwargs[kw])
                self._default.pop(kw)
            else:
                print("Ignoring unknown attribute '{}'!".format(kw))
        # assign remaining default attributes
...
class ADS1115(I2c_device):
    ...
    def get_single(self,ch=None,MUX=None):
    ...
    def get(self):
        ...
        if self.cycle == None:
            ...
            # read, setting MUX to next in cycle; advance cycle
...
# ----- LSM9DS1_ACC: iNEMO interial module: 3D accelerometer, ST -----
class LSM9DS1_MAGMA(I2c_device):
    """@@ To do ... """
    ...
    CONF_REG = {\
        ...
        'ODR':(0x20,2,3,'Output data rate',\
               [0.625,1.25,2.5,5.0,10.0,20.0,40.0,80.0]),\
        'ST':(0x20,0,1,'Self test',[0,1]),\
        'FS':(0x21,5,2,'Full scale',[8.0,16.0,24.0,32.0]),\
        'REBOOT':(0x21,3,1,'Reboot memory content',[0,1]),\
        'SOFT_RST':(0x21,2,1,'Register reset function',[0,1]),\
        'I2C_DISABLE':(0x22,7,1,'Disable I2C interface',[0,1]),\
        'LP':(0x22,3,1,'Low-power mode',[0,1]),\
    ...
    # Status register (1 x 8Bit, see datasheet)   
    STAT_REG = {\
        'ZYXOR':(0x27,7,1,'X,Y and Z overrun'),\
        'ZOR':(0x27,6,1,'Z overrun'),\
        'YOR':(0x27,5,1,'Y overrun'),\
    ...
    @property
    def full_scale(self):
        " Full scale setting (in Gauss). "
        return self._FS
    @full_scale.setter
    def full_scale(self,value):
        assert value in self.CONF_REG['FS'][4],"Invalid setting!"
        val = self.CONF_REG['FS'][4].index(value)
        self.config(FS=val)
        self._FS = value
    ...
    def data_rate(self,value):
        assert value in self.CONF_REG['ODR'][4],"Invalid setting!"
        val = self.CONF_REG['ODR'][4].index(value)
        self.config(ODR=val)
        self._ODR = value        

    def __init__(self,**kwargs):
        I2c_device.__init__(self,**kwargs)
        ans = self.read(ctrl=0x0f)
        assert ans[0] == 0b00111101,\
               "I am not a magnetometer!"

    def get_status(self,**args):
        " Returns the status of the conversion register. "
        if len(args) == 0:
            args = list(self.STAT_REG)
        for kw in args:
            assert kw in self.STAT_REG,"Unknown status key, {}!".format(kw)
        ans = self.read(ctrl=0x27)[0]
        out = {}
        for kw in args:
            out[kw] = bit_grab(ans,self.STAT_REG[kw][1],\
                               self.STAT_REG[kw][2])
        return out
        
    def get_output(self,axis=0):
        " Returns the measurement output along 'axis'. "
        if axis == 0:
            # read from x-registers
            LSb = self.get_raw('XLO')
            MSb = self.get_raw('XHI')
            ...
            LSb = self.get_raw('ZLO')
            MSb = self.get_raw('ZHI')
        return self._FS*twoscompl(MSbLSb2int(MSb,LSb),n=16)/(2**16)

    def get(self,axis=2):
        " Default 'get' function: z-axis reading. "
        return self.get_output(axis)        
...
class DAC8574(I2c_device):
    ...
    def set_output(self,ch,value,units=None):
        " Sets the output of channel ch (0-3 = A-D) to value.\
        ch=-1 can be used to simultaneously set all channels."
        self.write_DAC_data(ch,value,load=[0,1],units=units)
...
if __name__ == "__main__":
    ...
    adc = ADS1115(addr=0x4a)
    adc.cycle=[adc.CH0,adc.CH1,adc.CH2,adc.CH3]
    mag = LSM9DS1_MAG(addr=0x1e,cycle=[0,1,2])
    acc = LSM9DS1_ACC(addr=0x6b,cycle=[10,11,12,0,1,2,20])
    acc.config(ODR_XL=0b001,ODR_G=0b001)
    hih = HIH8121(addr=0x27,cycle=[0,1])
    meas_list = [adc,adc,adc,adc,acc,acc,acc,acc,acc,acc,acc,hih,hih]
    while True:
        print(" , ".join(["{:.4f}".format(dev.get())
               for dev in meas_list]))
        time.sleep(0.1)


    #while True:
    #    print("{:.4f} , {:.4f}".format(acc.get_acc(FS=1.0),acc.get_temp(FS=1.0)))
    #adc = ADS1115(addr=0x4b,bus=smbus.SMBus(1))
    #print(adc)
    #adc.config_info()
    #print(adc.config())
    #adc.config(DR=3,MODE=0,PGA=2,MUX=5)
    #print(adc.config())
    #while(True):
    #    print(adc.get_single())
